...
const { validateUsersConfig, safeAccountLabel } = require('./runtime_helpers');
...
const { proxyFromEnv } = require('./proxy_manager');
const {
    bodyText,
    expiryFromText,
    notReadyFromText,
...
async function preflightProxy(proxy) {
    ...
    try {
        ...
        return require('./runtime_helpers').classifyProxyResponse(response.status);
    } catch (error) {
        return error.response?.status
            ? require('./runtime_helpers').classifyProxyResponse(error.response.status)
            : require('./runtime_helpers').classifyProxyError(error);
    }
}
...
async function waitForRenewCompletion(page, modal, oldExpiry) {
    const deadline = Date.now() + CONFIG.renewSettleMs;
    let cfHandled = false;
    let retriedAfterCf = false;

    while (Date.now() < deadline) {
        const state = await readRenewState(page, modal);
        if (state.success) return { status: 'success', message: 'Renew successful', oldExpiry, newExpiry: state.expiry, cfHandled };
        if (state.notReady) return { status: 'not_ready', message: state.notReady, oldExpiry, newExpiry: state.expiry, cfHandled };
        if (oldExpiry && state.expiry && oldExpiry !== state.expiry) return { status: 'success', message: 'Expiry changed', oldExpiry, newExpiry: state.expiry, cfHandled };

        if (!cfHandled && await pageHasTurnstile(page, modal)) {
            cfHandled = true;
            console.log('[Renew阶段] Confirm 后检测到真实 Cloudflare Turnstile。');
            const solved = await solveTurnstile(page, 'Renew阶段', CONFIG, sleep);
            if (!solved.solved) {
                const info = await turnstileDiagnostics(page);
                return {
                    status: 'captcha_required',
                    message: `Renew Turnstile could not be completed${info.verificationFailed ? ': verification failed' : ''}`,
                    oldExpiry,
            ...
            await sleep(1_500);
            if (!retriedAfterCf && await modal.isVisible().catch(() => false)) {
                ...
                if (retry && await retry.isVisible().catch(() => false) && await retry.isEnabled().catch(() => false)) {
                    retriedAfterCf = true;
                    console.log('[Renew阶段] Cloudflare 验证完成，再次点击 Confirm。');
                    await retry.click();
                }
            }
            continue;
        ...
        if (!(await modal.isVisible().catch(() => false))) {
            ...
            if (oldExpiry && finalExpiry === oldExpiry) return { status: 'already_renewed', message: 'Renew modal closed but Expiry did not change', oldExpiry, newExpiry: finalExpiry, cfHandled };
            return { status: 'success', message: 'Renew modal closed after confirmation', oldExpiry, newExpiry: finalExpiry, cfHandled };
        }
    ...
    if (finalState.success || (oldExpiry && finalState.expiry && oldExpiry !== finalState.expiry)) return { status: 'success', message: 'Renew successful', oldExpiry, newExpiry: finalState.expiry, cfHandled };
    if (finalState.notReady) return { status: 'not_ready', message: finalState.notReady, oldExpiry, newExpiry: finalState.expiry, cfHandled };
    ...
    return { status: 'error', message: 'Renew confirmation completed without a clear result', oldExpiry, newExpiry: finalState.expiry, cfHandled };
}
...
async function login(page, user) {
    try {
        ...
        if (/Incorrect password or no account/i.test(afterLogin)) return { ok: false, status: 'login_failed', message: 'Incorrect password or no account' };
        if (/error=captcha/i.test(page.url()) || /Please complete captcha|captcha required/i.test(afterLogin)) return { ok: false, status: 'login_captcha_required', message: 'Login captcha was not accepted' };
...
async function renewAccount(page) {
    ...
    const before = await readRenewState(page, modal);
    if (before.notReady) return { status: 'not_ready', message: before.notReady, oldExpiry: before.expiry };

    // 任何 Protected by ALTCHA / altcha-widget / 普通 checkbox 都不触发验证。
    // 只有真实可见 Turnstile challenge 才处理。
    if (await pageHasTurnstile(page, modal)) {
        console.log('[Renew阶段] 检测到真实 Cloudflare Turnstile，尝试处理。');
        const solved = await solveTurnstile(page, 'Renew阶段', CONFIG, sleep);
        if (!solved.solved) return { status: 'captcha_required', message: 'Renew Turnstile could not be completed', oldExpiry: before.expiry };
    } else {
        console.log('[Renew阶段] 未检测到真实 Cloudflare Turnstile，正常继续。');
    }

    const confirm = await getRenewConfirmButton(modal);
    if (!confirm || !(await confirm.isVisible().catch(() => false))) return { status: 'error', message: 'Renew confirm button not found', oldExpiry: before.expiry };
    if (!(await confirm.isEnabled().catch(() => false))) return { status: 'error', message: 'Renew confirm button is disabled', oldExpiry: before.expiry };

    await confirm.click();
    console.log(`[Renew阶段] Confirm 已点击，等待约 ${Math.round(CONFIG.renewSettleMs / 1000)} 秒。`);
    return waitForRenewCompletion(page, modal, before.expiry);
}

async function runAccount(context, user, index) {
    ...
    try {
        // 清理同源 Web Storage，避免多账号在同一个 CDP context 中串状态。
        ...
        const renewResult = await renewAccount(page);
        let debug = {};
        if (renewResult.status === 'success') debug = await dump(page, `renew_success_${label}`);
        else if (renewResult.status !== 'already_renewed') debug = await dump(page, `renew_${renewResult.status}_${label}`);
...
async function run() {
    ...
    try {
        ...
        for (let i = 0; i < usersConfig.users.length; i += 1) {
            const account = await runAccount(context, usersConfig.users[i], i);
        ...
        const finalResult = makeResult({
            exitCode: overall,
            status: summarizeStatus(accounts),
            message: accounts.at(-1)?.message || '',
            accounts,
            screenshotPath,
            htmlPath,
        });
...
module.exports = { EXIT_CODE, CONFIG, proxyFromEnv, expiryFromText, notReadyFromText, run };
