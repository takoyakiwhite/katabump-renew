...
async function waitForRenewCompletion(page, modal, oldExpiry) {
    ...
    while (Date.now() < deadline) {
        ...
        if (!(await modal.isVisible().catch(() => false))) {
            ...
            if (oldExpiry && finalExpiry === oldExpiry) {
                return { status: 'already_renewed', message: 'Renew modal closed but Expiry did not change', oldExpiry, newExpiry: finalExpiry, cfHandled };
            }
            if (oldExpiry && finalExpiry && oldExpiry !== finalExpiry) {
                return { status: 'success', message: 'Expiry changed', oldExpiry, newExpiry: finalExpiry, cfHandled };
            }
            if (successFromText(finalText)) {
                return { status: 'success', message: 'Renew successful', oldExpiry, newExpiry: finalExpiry, cfHandled };
            }
            return { status: 'error', message: 'Renew modal closed without a verifiable renewal result', oldExpiry, newExpiry: finalExpiry, cfHandled };
        }
...
module.exports = { EXIT_CODE, CONFIG, proxyFromEnv, expiryFromText, notReadyFromText, successFromText, run };
