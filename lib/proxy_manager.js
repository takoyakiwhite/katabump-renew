...
function parseProxyLine(line, lineNumber) {
    ...
    if (trimmed.startsWith('http://')) {
        ...
        const port = explicitPort || parsed.port || '80';
        if (parsed.protocol !== 'http:' || !parsed.hostname || parsed.pathname !== '/' || parsed.search || parsed.hash || !parsed.username || !parsed.password || !isValidHost(parsed.hostname) || !isValidPort(port)) {
...
function buildHttpProxy(parsed) {
    ...
    try {
        ...
        if (url.protocol !== 'http:' || url.hostname !== parsed.ip || effectivePort !== String(parsed.port)) return null;
        return proxyUrl;
    } catch {
        return null;
    }
...
function proxyKey(parsed) { return `${parsed.ip}:${parsed.port}`; }
function safeProxyId(parsed) { return parsed?.valid ? proxyKey(parsed) : 'invalid'; }

function maskProxyUrl(proxyUrl) {
    try {
        const url = new URL(proxyUrl);
        const port = url.port || (url.protocol === 'http:' ? '80' : '');
        return url.username || url.password ? `${url.protocol}//***:***@${url.hostname}:${port}` : proxyUrl;
    } catch {
        return '***';
    }
}
...
function buildProxyCandidateQueue(proxies, cooldowns, attemptedKeys = new Set()) {
    ...
    for (const proxy of proxies) {
        ...
        if (cooldowns[key] && Number(cooldowns[key].until) > now) continue;
        seen.add(key);
...
function selectRandomProxy(proxies, cooldowns = {}) {
    return buildProxyCandidateQueue(proxies, cooldowns)[0] || null;
}
