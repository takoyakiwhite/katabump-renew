...
// 仅表示当前页面存在可见、可交互的 challenge，不把已有 token 当成 challenge。
async function hasActiveTurnstile(page, scope = page) {
...
async function pageHasTurnstile(page, scope = page) {
    return hasActiveTurnstile(page, scope);
}
...
async function solveTurnstile(page, stage, config, sleep) {
    ...
    if (!(await hasActiveTurnstile(page))) return { present: false, solved: false, attempts: 0 };
    ...
    for (let attempt = 1; attempt <= config.turnstileRetryLimit; attempt += 1) {
        ...
        while (Date.now() < deadline) {
            ...
            if (!(await hasActiveTurnstile(page))) return { present: true, solved: true, attempts: attempt, info };
