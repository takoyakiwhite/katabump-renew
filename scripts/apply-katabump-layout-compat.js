...
const replacements = [
    ...
    { file: 'action_renew.js', from: "const confirmBtnAfterCb = modal.getByRole('button', { name: 'Renew' });", to: "let confirmBtnAfterCb = modal.locator('div.modal-footer button.btn.btn-primary').filter({ hasText: /^Renew$/i }).first();\n                            if (!(await confirmBtnAfterCb.isVisible().catch(() => false))) {\n                                confirmBtnAfterCb = modal.getByRole('button', { name: 'Renew' }).first();\n                            }" },
    ...
    { file: 'action_renew.js', from: "await confirmBtn.click();\n                    console.log('Confirm Renew clicked.');\n\n                    // 点击后等待响应\n                    await page.waitForTimeout(2000);", to: "const renewConfirmResult = await confirmRenewWithDynamicCloudflare(page, modal, confirmBtn);\n                    console.log('[Renew阶段] Confirm 流程完成，动态 Cloudflare=' + (renewConfirmResult.cloudflareHandled ? '已处理' : '未出现'));" }
];
